...
public class HidingManager : MonoBehaviour
{
    ...
    public bool ReportPlayerHidden(string hideSpotId)
    {
        ...
        RegisterHideSpotUse(hideSpotId);
    ...
    private void RegisterHideSpotUse(string hideSpotId)
    {
        if (string.IsNullOrWhiteSpace(hideSpotId) || hideSpotUsesPerStage <= 0)
        {
            return;
        }

        int currentCount = GetHideSpotUseCount(hideSpotId);
        hideSpotUseCounts[hideSpotId] = currentCount + 1;
    }
